...
class _PerformanceMeter(object):
    ...
    def get_score(self):
        with torch.no_grad():
            for tn, task in enumerate(self.task_name):
                self.results[task] = self.metrics[task].score_fun() # self.abs_record和self.rel_record相关
    ...
    def display(self, mode, epoch):
        ...
        task_mse = {}
        for tn, task in enumerate(self.task_name):   #输入的三个数值含义分别为平均MSE、平均绝对误差、平均相对误差
            # task_mse[task] = self.loss_item[tn] #  self.results[task]代表accuracy+absloss,而self.loss_item[tn]代表二元交叉熵
            if task != "phase_class":
                task_mse[task] = self.loss_item[tn]
            else:
                task_mse[task] = self.results[task][0]
        ...
        print(' | ', end='') if mode!='val' else print() # 根据mode的不同在这里对应修改是为val或者test
        return task_mse
    ...
    def _update_best_result(self, new_result, epoch):
        improvement = count_improvement(self.base_result, new_result, self.weight)
        print("Base result: %s; new_result: %s" % (self.base_result,new_result))
        self.improvement = improvement
        if improvement > self.best_result['improvement']:
            self.best_result['improvement'] = improvement
            self.best_result['epoch'] = epoch
