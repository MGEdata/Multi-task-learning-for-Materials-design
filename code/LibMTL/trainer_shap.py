...
class Trainer_shap(nn.Module):
    ...
    def __init__(self, task_dict, weighting, architecture, encoder_class, decoders, rep_grad, multi_input,
        ...
        super(Trainer_shap, self).__init__()
    ...
    def train(self, train_dataloaders, data_train,data_test,data_all, epochs, fig_id, weights,
        ...
        for epoch in range(epochs):
            ...
            if not self.multi_input:
                ...
                for batch_index in range(train_batch):
                    ...
                    for tn, task in enumerate(self.task_name):
                        ...
                        ori_train_pred,features = self.model([train_input, train_plus], task)
            ...
            def predict(all_X_train):
                ...
                # train_loader, train_batch = self._prepare_dataloaders(train_dataloaders)
                # train_batch = max(train_batch) if self.multi_input else train_batch
                all_X_train = torch.tensor(all_X_train)
                with torch.no_grad():
                    print("shape of data_train")
                    print(all_X_train.shape)
                    # return self.model([all_X_train,torch.tensor(np.full((all_X_train.shape[0], 4), [1300, 24, 1100, 168]))], 'size')[0]['size'].numpy()
                    # return self.model([all_X_train, torch.tensor([])], 'size')[0]['size'].numpy()
                    return self.model([all_X_train, torch.tensor(np.full((all_X_train.shape[0], 4), [1225,24,900,24]))],'size')[0]['size'].numpy()

            if epoch == 119:
                print("Shap parsing begin")
                # 从DataLoader中提取数据并转换为NumPy数组
                all_X_train = []
                all_X_train_plus = []
                all_y_train = []
                for i, (x,x_plus,y) in enumerate(train_dataloaders['size']):
                    # print(f"Batch {i}:")
                    x = x.numpy()
                    x_plus = x_plus.numpy()
                    y = y.numpy()
                    all_X_train.append(x)
                    all_X_train_plus.append(x_plus)
                    all_y_train.append(y)

                all_X_train = np.array(all_X_train)

                all_X_train_plus = np.array(all_X_train_plus)
                all_y_train = np.array(all_y_train)

                all_X_train = np.reshape(all_X_train,(96,24))# 第一个数值根据报错size/24来确定

                all_y_train = np.reshape(all_y_train,(96))
                # # 将列表中的批次数据拼接起来
                # all_X_train = np.concatenate(all_X_train, axis=0)
                # all_y_train = np.concatenate(all_y_train, axis=0)
                # 创建一个SHAP解释器

                explainer = shap.Explainer(predict, all_X_train)

                shap_values = explainer(all_X_train)
                mean_abs_shap_values = np.mean(np.abs(shap_values.values), axis=0)

                # np.save(r'C:\Users\Administrator\Desktop\wwr-files\multi-task\code\frame2_creep\shap_data\phase_class_ma_shapvalues.npy',mean_abs_shap_values)
                import pandas as pd
                feature_names = ['Co', 'Al', 'W', 'Ni', 'Ti', 'Cr', 'Ge', 'Ta', 'B', 'Mo', 'Re', 'Nb', 'Mn', 'Si','V', 'Fe', 'Zr', 'Hf', 'Ru', 'Ir', 'La', 'Y', 'Mg', 'C']
                all_X_train = pd.DataFrame(all_X_train, columns=feature_names)
                shap_values_df = pd.DataFrame(shap_values.values, columns=all_X_train.columns)
                # shap_values_df.to_csv(r'C:\Users\Administrator\Desktop\wwr-files\multi-task\code\frame2_creep\shap_data\phase_class.csv', index=False)
                # 可视化前几个样本的SHAP值
                # shap.summary_plot(shap_values, all_X_train)
                shap.summary_plot(shap_values, all_X_train, feature_names=feature_names)
                shap.summary_plot(shap_values, all_X_train, plot_type="bar", feature_names=feature_names)
                print("Shap parsing over")
            ...
            #     print("3")
            #     shap.summary_plot(shap_values, [comps_data, plus_data], plot_type="bar", plot_size=[15, 10])
